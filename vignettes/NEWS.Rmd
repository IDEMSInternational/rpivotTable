...
#### News
**Information on Version 0.4 is authored by Lily Clements:**
...
* IDEMS International have taken over as maintainer
* `rpivotTable()` now automatically generates `sorters` from the factor levels of any factor columns in `data`, so rows/columns sort in factor-level order rather than alphabetically by default; pass your own `sorters` argument to override
* Various fixes for CRAN resubmission (encoding declaration, Rd markup, vignette compatibility with current dplyr)
